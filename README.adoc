...
First download jla_01 from following url. The dependencies for jla_01 are now included into the jla_01.jar as shaded ones:

https://search.maven.org/artifact/com.teragrep/jla_01[jla_01]

Then run java while pointing classpath to the directory where you downloaded the jar to like:

```
java -cp "path/to/downloaded/jars/jla_01.jar:target/example.jar" com.teragrep.example.Main
```
